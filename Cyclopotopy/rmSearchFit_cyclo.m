function view = rmSearchFit_cyclo(view, params, doDecimate, varargin)
...
% 2013/07  MB: adapted to special version for cyclopotopy project
...
for slice=loopSlices,
    ...
    for n=1:numel(model)
        ...
        % BR: When refitting, leave s{n}.b(3) unaffected
        reset = true;
        if isfield(params.analysis.fmins,'refine') && strcmpi(params.analysis.fmins.refine,'refit') && ...
           isfield(params.analysis.cyclo,'refitBeta') && ~params.analysis.cyclo.refitBeta
            reset = false;
        end
        ...
        switch desc,
            case {'2d prf fit (x,y,sigma, positive only)','2'}
                ...
                if isfield(params.analysis,'xlim') && params.analysis.xlim~=0
                    ...
                    elseif isfield(params.analysis.fmins,'refine') && strcmpi(params.analysis.fmins.refine,'refit');
                        s{n}=rmSearchFit_oneGaussianCyclo(s{n},data,params,wProcess,t);
                        
                        if isfield(params.analysis.cyclo,'refitBeta') && ~params.analysis.cyclo.refitBeta
                           trendBetas = zeros(size(trendBetas)); % Betas weren't reset so don't add them to data                            
                        end
            ...
            case {'difference 2d prf fit (x,y,sigma,sigma2, center=positive)'}
                ...
                elseif isfield(params.analysis.fmins,'refine') && strcmpi(params.analysis.fmins.refine,'refit');
                    s{n}=rmSearchFit_twoGaussiansDoG_fixed(s{n},data,params,wProcess,t);   
